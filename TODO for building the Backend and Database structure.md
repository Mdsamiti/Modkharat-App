...
- [x] Implement async ingestion pipelines for OCR and Voice, plus SMS parsing and SMS sending.
...
- [x] Use Supabase Storage buckets: `receipts`, `voice-recordings`, `exports`. *(Done — Phase 2)*
...
- [x] `IngestionJobDTO`. *(Done — Phase 2)*
...
- [x] Add ingestion job status enum: `queued | processing | completed | failed`. *(Done — Phase 2)*
...
- [x] Create `transaction_sources` table for raw capture payloads and parse confidence. *(Done — Phase 2)*
...
- [x] Create `ingestion_jobs` table to track OCR/voice/SMS parse pipelines and outputs. *(Done — Phase 2)*
- [x] Create `sms_messages` table for raw bank SMS text, sender, parse output, and confidence. *(Done — Phase 2)*
...
## 7. OCR Integration (Google Vision) — *Done (Phase 2)*
- [x] Add endpoint `/v1/import/ocr` to accept receipt image upload and create ingestion job.
- [x] Store image in Supabase Storage and record file metadata/hash for dedupe.
- [x] Worker flow: fetch image -> call Vision OCR -> normalize text -> parse merchant/amount/date/currency -> create `draft` transaction.
- [x] Persist raw OCR text and parser confidence to `transaction_sources`/`ingestion_jobs`.
- [x] Return job ID immediately; frontend polls `/v1/import/jobs/:jobId`.
- [x] Add fallback behavior for low-confidence results requiring manual review.

## 8. Voice Integration (Google Speech-to-Text) — *Done (Phase 2)*
- [x] Add endpoint `/v1/import/voice` to upload audio or receive URI and enqueue transcription job.
- [x] Use language hints for `ar-SA` and `en-US`; persist transcript and confidence.
- [x] Parse transcript into transaction draft with deterministic extraction rules.
...
- [x] Run separate worker process (`backend worker`) for OCR/voice/SMS background jobs.
- [x] Implement retries with exponential backoff and dead-letter handling.
- [x] Track SLA metrics: queue delay, processing time, success/failure counts.
...
- [ ] DB integration tests on ephemeral PostgreSQL with migrations applied. *(Ops — needs Docker/Testcontainers setup)*
- [x] Provider adapter tests with mocked Google/Twilio responses and timeout/failure cases. *(Done — Phase 2)*
...
- [ ] Apply least privilege to DB/API credentials and rotate keys regularly. *(Ops — post-deployment)*
...
- [x] SMS parse creates draft transaction from realistic Saudi bank message format. *(Done — Phase 2)*
- [x] OCR receipt import returns job ID, then job completes with parsed draft transaction. *(Done — Phase 2)*
- [x] Voice import handles Arabic utterance and returns structured draft transaction. *(Done — Phase 2)*
...
- [x] Family invite via Twilio SMS records sent status and webhook delivery update. *(Done — Phase 2)*
