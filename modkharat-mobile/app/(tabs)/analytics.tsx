...
import {
  mockSpendingByCategory, mockIncomeVsExpenses, mockBudgets,
...
export default function AnalyticsScreen() {
  ...
  const savingsRate = (((mockIncome - mockExpenses) / mockIncome) * 100).toFixed(1);
  const totalSpending = mockSpendingByCategory.reduce((s, c) => s + c.value, 0);
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row px-4 mt-4 gap-3">
          <View className="flex-1 bg-emerald-50 rounded-2xl p-4" accessibilityLabel={`${t('analytics.totalIncome')}: ${currency} ${mockIncome.toLocaleString()}`}>
            <View className="flex-row items-center mb-2">
              <TrendingUp size={16} color="#059669" />
              <Text className="text-emerald-700 text-xs ml-1" numberOfLines={1}>{t('analytics.totalIncome')}</Text>
            </View>
            <Text className="text-emerald-800 font-bold text-lg" numberOfLines={1}>{currency} {mockIncome.toLocaleString()}</Text>
          </View>
          <View className="flex-1 bg-red-50 rounded-2xl p-4" accessibilityLabel={`${t('analytics.totalExpenses')}: ${currency} ${mockExpenses.toLocaleString()}`}>
            <View className="flex-row items-center mb-2">
              <TrendingDown size={16} color="#ef4444" />
              <Text className="text-red-700 text-xs ml-1" numberOfLines={1}>{t('analytics.totalExpenses')}</Text>
            </View>
            <Text className="text-red-800 font-bold text-lg" numberOfLines={1}>{currency} {mockExpenses.toLocaleString()}</Text>
          </View>
        </View>
        ...
        <View className="mx-4 mt-4 bg-white rounded-2xl p-4">
          ...
          {/* Donut Representation as horizontal bars */}
